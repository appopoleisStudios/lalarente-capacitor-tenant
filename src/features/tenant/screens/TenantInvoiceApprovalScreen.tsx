/**
 * Tenant Invoice Approval Screen (Plane #109)
 *
 * Tenant sees invoices where payer_role='tenant' and can approve or reject
 * with a written reason. Mirrors the owner's OwnerInvoiceApprovalScreen but
 * scoped to the tenant's active leases.
 */

import { useAuth } from '@/src/contexts/AuthContext';
import {
  getInvoicesByRequest,
  tenantApproveInvoice,
  tenantRejectInvoice,
...
export default function TenantInvoiceApprovalScreen() {
  ...
  const loadInvoices = async () => {
    ...
    try {
      ...
      if (!abortRef.current?.signal.aborted) {
        // Only show invoices billed to tenant
        setInvoices(
          (data || []).filter(
            (inv) =>
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              await tenantApproveInvoice(invoice.id, user.id);
              Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
              Alert.alert('Approved', 'Invoice approved. The vendor has been notified.');
              loadInvoices();
            } catch (error: any) {
  ...
  const submitRejection = async () => {
    ...
    try {
      ...
      if (!user?.id) throw new Error('Not authenticated');
      await tenantRejectInvoice(selectedInvoice.id, user.id, rejectReason.trim());

      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert('Rejected', 'The vendor has been notified of your rejection.');
  ...
  const renderInvoice = (invoice: MaintenanceInvoice) => {
    ...
    const canAct = invoice.status === 'submitted';
    ...
    return (
      <View key={invoice.id} style={styles.invoiceCard}>
        ...
        {/* Previous rejection reason (from owner or prior tenant rejection) */}
        ...
        {canAct && (
          <View style={styles.actions}>
            <TouchableOpacity
              ...
              testID="tenant-invoice-reject-button"
            >
            ...
            <TouchableOpacity
              ...
              disabled={actionLoading}
              testID="tenant-invoice-approve-button"
            >
              {actionLoading ? (
        ...
        {invoice.status === 'approved' && (
          <View style={styles.approvedBanner}>
            ...
            <Text style={styles.approvedText}>
              Approved {formatDate(invoice.approved_at)}. You can now pay this invoice from the
              Payments tab.
            </Text>
          </View>
        )}
        ...
        {invoice.status === 'paid' && (
          <View style={styles.paidBanner}>
            <Ionicons name="cash" size={18} color={colors.success[700]} />
            <Text style={styles.paidText}>Paid {formatDate(invoice.paid_at)}.</Text>
          </View>
        )}
      </View>
    );
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <View style={styles.headerButton}>
          <FeaturePin
            pinId="tenant-invoice-approve"
            title="Approving an invoice"
            message="Review the line items carefully. Approve if the work matches what was done. Reject with a reason if something is wrong — the vendor can fix and resubmit."
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>
            There are no vendor invoices billed to you for this maintenance request.
          </Text>
        </View>
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              Explain why you are rejecting this invoice. The vendor will be notified and can fix
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                ...
                testID="tenant-invoice-reject-confirm"
              >
...
const styles = StyleSheet.create({
  ...
  totalAmount: { fontSize: 16, fontWeight: '800', color: RSA.green },
  ...
  approveButton: { backgroundColor: RSA.green, borderColor: RSA.green },
  ...
  paidText: { flex: 1, fontSize: 13, color: colors.success[700] },
