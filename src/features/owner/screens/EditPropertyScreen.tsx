  ...
  const loadPropertyData = async () => {
    try {
      ...
      setForm({
        ...
        minimum_lease_months: property.minimum_lease_months?.toString() || '12',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await propertiesApi.updateProperty(id, {
        ...
        minimum_lease_months: form.minimum_lease_months
          ? parseInt(form.minimum_lease_months)
          : null,
                  ...
                  <AnimatedButton key={option.value} onPress={() => toggleAmenity(option.value)}>
                    <View style={styles.checkbox}>
                      <View
                        style={[
                          styles.checkboxBox,
                          form.amenities.includes(option.value) && styles.checkboxChecked,
                        ]}
                      >
                        {form.amenities.includes(option.value) && (
                  ...
                  <AnimatedButton key={option.value} onPress={() => toggleService(option.value)}>
                    <View style={styles.checkbox}>
                      <View
                        style={[
                          styles.checkboxBox,
                          form.services_provided.includes(option.value) && styles.checkboxChecked,
                        ]}
                      >
                        {form.services_provided.includes(option.value) && (
                          <Text style={styles.checkmark}>✓</Text>
                        )}
                      </View>
                      <Text style={styles.checkboxLabel}>{option.label}</Text>
                ...
                <View style={styles.tagsContainer}>
                  {form.services_provided.map((service) => (
                    <AnimatedButton key={service} onPress={() => removeService(service)}>
                      <View
                        style={[styles.tag, { backgroundColor: '#d1fae5', borderColor: '#059669' }]}
                      >
