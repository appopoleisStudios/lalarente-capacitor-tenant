...
import { requestClosure } from '@/src/features/maintenance/api';
...
export default function RequestClosureScreen() {
  ...
  const minPhotosRequired = 2;
  const hasMinPhotos = files.length >= minPhotosRequired;

  const handleSubmit = async () => {
    if (!completionNotes.trim()) {
      Alert.alert('Notes Required', 'Please describe the work completed.');
      return;
    }
    if (!hasMinPhotos) {
      Alert.alert('Photos Required', `Please upload at least ${minPhotosRequired} completion photos.`);
      return;
    }
    if (!user?.id) {
      Alert.alert('Error', 'You must be logged in to request closure.');
      return;
    }
    ...
    try {
      ...
      // Request closure
      await requestClosure(id, user.id, completionNotes.trim(), photoUrls);

      Alert.alert(
        '✅ Closure Requested',
        'Your job closure request has been sent to the owner for review.',
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Request Job Closure</Text>
        ...
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.instructionCard}>
            ...
            <View style={styles.instructionTextWrap}>
              <Text style={styles.instructionTitle}>Ready to wrap up?</Text>
              <Text style={styles.instructionText}>
                Summarise the completed work and attach at least {minPhotosRequired} photos showing the final result.
          ...
          <View style={styles.section}>
            ...
            <TextInput
              ...
              placeholder="e.g. All work completed as per PO. Bathroom tiling finished, plumbing tested and working..."
          ...
          <View style={styles.section}>
            <Text style={styles.label}>
              Completion Photos * ({files.length}/{minPhotosRequired} minimum)
            </Text>
            <Text style={[styles.hint, !hasMinPhotos && files.length > 0 && styles.hintWarning]}>
              {hasMinPhotos
                ...
                : files.length > 0
                  ...
                  : `Upload at least ${minPhotosRequired} photos of the completed work`
            }
            </Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[
            styles.submitButton,
            (!completionNotes.trim() || !hasMinPhotos || submitting) && styles.submitButtonDisabled,
          ]}
          onPress={handleSubmit}
          disabled={!completionNotes.trim() || !hasMinPhotos || submitting}
        >
          {submitting ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>Submit Closure Request</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
