  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (response.ok && data) {
          ...
          if (data.payment_status === 'completed' || data.payment_status === 'failed' || data.payment_status === 'cancelled') {
            if (pollRef.current) {
    ...
    setTimeout(() => {
      if (pollRef.current) {
        ...
        setLoading(false);
        if (!status) {
          setError('Payment confirmation is taking longer than expected. Your payment may still be processing.');
        }
      }
    }, 30000);
        ...
        <View style={{ width: '100%', gap: 12 }}>
          {isSuccess ? (
            <>
              <TouchableOpacity
                style={{
                  backgroundColor: '#007A4D', borderRadius: 10, padding: 16,
                  alignItems: 'center',
                }}
            ...
            <>
              <TouchableOpacity
                style={{
                  backgroundColor: '#DE3831', borderRadius: 10, padding: 16,
                  alignItems: 'center',
                }}
                onPress={() => router.push('/(tenant)/vendor-payments')}
              >
